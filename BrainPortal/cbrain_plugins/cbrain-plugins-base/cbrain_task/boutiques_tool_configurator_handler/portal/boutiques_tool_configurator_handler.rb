...
#puts_red "C=#{param} A='#{self.invoke_params[param].inspect}' B='#{newvalue.inspect}'"
    ...
    if (old_tc && new_tc && old_tc.id != new_tc.id)
      ...
      change.(:copy_overlays,
         (old_tc.apprainer_overlays_specs.present? ||
          old_tc.container_exec_args.present? ||
          (old_tc.apprainer_use_short_workdir? != new_tc.apprainer_use_short_workdir?)
         ),
  ...
  def copy_attributes
    ...
    if bool.(invoke_params[:copy_overlays])
      new_tc.apprainer_overlays_specs    = old_tc.apprainer_overlays_specs
      new_tc.apprainer_use_short_workdir = old_tc.apprainer_use_short_workdir
    ...
    new_tc.save_with_logging((self.user || CoreAdmin.admin),
      ...
      %i(
          ...
          env_array
            script_prologue
            script_epilogue
          apprainer_overlays_specs
          apprainer_use_short_workdir
          container_exec_args
  ...
  def add_all_input_notes(descriptor) #:nodoc:
    ...
    cp_ovrl.cbrain_input_notes << "OLD ToolConfig has overlays" if old_tc&.apprainer_overlays_specs.present?
    cp_ovrl.cbrain_input_notes << "OLD ToolConfig uses short workdirs" if old_tc&.apprainer_use_short_workdir?
    cp_ovrl.cbrain_input_notes << "OLD ToolConfig uses special container options" if old_tc&.container_exec_args.present?
    cp_ovrl.cbrain_input_notes << "NEW ToolConfig has overlays" if new_tc&.apprainer_overlays_specs.present?
    cp_ovrl.cbrain_input_notes << "NEW ToolConfig uses short workdirs" if new_tc&.apprainer_use_short_workdir?
